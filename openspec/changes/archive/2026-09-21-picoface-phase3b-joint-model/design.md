...
- **"Classifies out of the box" is not demonstrable at default settings on the stub dataset.** At `train()`'s default 10 epochs, 16 stub images give only 10 optimizer steps: the joint VAE beat chance in 1/10 seeds (Arm 1's CNN: 3/10), versus 10/10 at 50 epochs. Not a joint-model defect, a stub-size artifact. → The spec scenario and test fix the epoch count at 50; Phase 6 must verify the default epochs on the real dataset, where each epoch has many more steps.
- **The existing reconstruction-loss test is a weak tripwire.** On the stub dataset (16 images, batch 16) each epoch is a single optimizer step; a 30-epoch smoke run showed recon loss improving only ~3% (0.0312 → 0.0303) though consistently (20/20 seeds), and the unseeded 5-epoch version flaked ~10% of runs before this change. → Seeded in tasks 1.9; new progress-asserting tests follow the same convention (fixed seed, first few epochs). A stronger regression signal needs more data per epoch, which is a Phase 6 concern.
