...
The installable-package structure and naming for `picoface`: aligning the
GitHub repo name, Python import name, and PyPI distribution name, and
structuring the project (`pyproject.toml` + src layout) so it supports
pip/zip/Colab-git-clone distribution without committing to one channel.
Dataset Forge is kept structurally separate so its dependencies never leak
into the installable package.

Phase 0 fully satisfies this capability's structural requirements below.
Choosing and documenting which distribution channel is actually used (PyPI
vs. zip vs. Colab clone) is deferred to Phase 7 and will extend this spec,
not replace it.
