...
**Non-Goals:**
- Changing decoder architecture (more conv layers/channels, label-conditioning). Explicitly deferred to a later change if tuning doesn't resolve the blobs (proposal.md, What Changes).
...
**Alternative considered and rejected for this change: go straight to decoder capacity.** The diagnostics record's own framing ("It is a Phase 6 tuning question... It is not a Forge or data-contract issue") and this session's explicit scoping decision (tuning first, architecture only if tuning doesn't get there) both point at trying the cheaper, already-anticipated fix before adding model capacity. Skipping straight to architecture would also make it unclear afterward whether the loss balance was ever actually the problem.
