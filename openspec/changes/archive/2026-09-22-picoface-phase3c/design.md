...
The learned uncertainty parameters are **not** frozen during the annealing warmup; they re-equilibrate as the KL weight rises. Freezing was drafted as the conservative default and was tested at implementation time (diagnostics record, Decision 8). On the stub it only postponed the parameters' approach to equilibrium, which already lags (Decision 4). After 30 epochs, frozen parameters were further from equilibrium and reconstruction was worse (MSE 0.026–0.028 against 0.022–0.025), with held-out accuracy unchanged.
...
The mitigation lever is a floor on each `log(sigma^2)`, which caps how large a task's weight can grow; it reintroduces a hand-tuned constant — a much weaker one than the per-dataset weight it replaces, since it only bounds a pathology rather than setting the balance. Because both `sigma`s live on per-pixel / per-label scales (Decision 5), the floor means the same thing at every image resolution. Its value is a Phase 6 tuning question; the diagnostics in Decision 8 are what make it visible. Recording the learned weights per epoch is not optional polish here, it is how this risk gets caught before a student's laptop finds it.

The floor also keeps the objective well-behaved near convergence. Above it, a task's gradient is `(1/2) * grad(L_i) / L_i`. For reconstruction, that grows as the residuals shrink. For classification, it stops decaying: it keeps pushing logit margins up without limit. At the floor, each task becomes an ordinary loss with a bounded constant weight (at most `1 / (2 * e^floor)` for reconstruction and `e^(-floor)` for classification), and its gradient vanishes normally.

**The floor is enforced on the stored parameter, not only inside the loss.** After every optimizer step, each log-variance parameter is clamped in place to be at least the floor. Clamping only the value the loss reads would leave the parameter free to drift below the floor under Adam's momentum, where its gradient is exactly zero. It could not come back if its task loss later rose, for example as the KL weight anneals up. The stub diagnostics show this overshoot, to −7.0 against a floor of −6. Clamping the stored value keeps the parameter at the floor, where its gradient still acts, so it can recover.

**Reaching equilibrium is a separate matter from what equilibrium is.** The analysis above holds only once each `log(sigma^2)` has caught up with its task loss, and before that the model trains under a different balance. The effective KL weight relative to plain MSE is `2 * sigma_r^2`: about 2 at the initial `sigma_r^2 = 1`, against about 0.06 at equilibrium on the stub. Adam moves a parameter whose gradient keeps one sign by about its learning rate per step, whatever the gradient's size. So at the shared learning rate of `1e-3`, reaching `log(sigma_r^2) ≈ -3.5` takes thousands of steps, far longer than a default run.

The log-variance parameters therefore train at **10× the learning rate** of the rest of the model (`LOG_VAR_LR_MULTIPLIER = 10`), and reach equilibrium after about 340 optimizer steps. This is a deliberately conservative value:
- On real data, with hundreds of steps per epoch, that is a few epochs.
- On the stub, at 12 steps per epoch, it is not reached within 10 or 30 epochs; 30× reaches it in about 140 steps and 100× in about 36 (diagnostics record).
- The multiplier is relative to the `learning_rate` passed to `train()`, so a lower rate slows the `sigma`s proportionally.

The value is revisited in Phase 6, once Phase 5 fixes the real dataset's steps per epoch. Until then, stub runs that report learned weights must train for enough steps, and the diagnostics must say when a run's weights had not yet caught up.

**No weight decay is applied to any parameter**, matching the rest of the library. This is recorded as a Phase 6 question rather than settled. If it is added:
- It should be decoupled weight decay (`AdamW`), not Adam's coupled L2 `weight_decay`, which interacts with Adam's per-parameter scaling.
- It must exclude the log-variance parameters, since decay pulls them towards `sigma^2 = 1`, the lagging state above.
- A specific reason to want it: once the classification log-variance sits at the floor, cross-entropy still pushes logit margins up with nothing to bound them, and decay on the classification head would.
...
- **Estimating each `sigma^2` in closed form from the current batch's loss** (`sigma_r^2 = MSE`, `sigma_c^2 = 2 * CE`, with the gradient stopped through them; the "optimal sigma" of Rybkin et al., 2021) — rejected: it reaches the same equilibrium without any lag, but it replaces the learned homoscedastic parameters with per-batch estimates. The learned parameters are kept and given a faster learning rate instead.
- **The log-variances at the shared learning rate** — rejected: they barely leave their initial value in a default-length run (diagnostics record), so the "learned" balance would in practice be a fixed one.
...
- **Scheduling** is the one extension the interface needs. Annealing (Decision 3) is a function of training progress, which `training_step` currently cannot see, and progress must be relative to the caller's `epochs` argument since students choose it. `train()` therefore informs the model of the current epoch and total epochs before each epoch (a no-op hook on `_Model`, overridden by the VAE). This keeps the schedule owned by the model and the loop model-agnostic.
- **Optimizer settings and post-step constraints** are the other two extensions, both needed by Decision 4. `train()` asks the model for its optimizer parameter groups, given the caller's `learning_rate`, instead of passing `model.parameters()` directly. By default this is one group at that rate; the VAE puts its log-variances in a second group at `LOG_VAR_LR_MULTIPLIER` times it. `train()` also calls a no-op hook after every optimizer step, which the VAE uses to clamp its log-variances to the floor. Both are defaults on `_Model` that the VAE overrides, so `train()` still does not branch on model type.
...
- **The learned weights may not have caught up by the end of a short run** (Decision 4: Adam moves them by about their learning rate per step). → The conservative 10× multiplier reaches equilibrium within a few epochs on data with hundreds of steps per epoch, but not on the small stub. Stub runs that assert on or record learned weights use enough steps, and Phase 6 retunes the multiplier on real data.
...
None blocking implementation. Left to implementation-time experiment and recorded for Phase 6: the latent dimensionality, the annealing schedule's length and shape (its end point is fixed at the ELBO weight of 1 by Decision 3), the `log(sigma^2)` floor, the log-variance learning-rate multiplier and whether any weight decay is applied (Decision 4), and the specific figures the stub renders. Consistent with how Phases 2 and 3 left their architecture constants to implementation-time tuning.
