...
- Actually *performing* a PyPI publish (creating the PyPI account, running `twine upload`) — that remains the user's own manual, future action, documented but not automated by this repo (`RELEASING.md`). *Preparing* for that release — PyPI metadata, `build`/`twine` tooling, choosing PyPI as the target channel — is in scope as of Phase 7.
...
| 7 | Student docs & MVP packaging | `packaging` (remainder) | **Done as scoped** — student docs (README) and PyPI publish-prep (`pyproject.toml` metadata, `RELEASING.md`) complete; real-dataset distribution to students and template notebooks are explicitly out of this MVP, per the user's scoping decision (see `picoface-phase7/design.md`) |
...
- ~~Concrete accuracy/quality bar for the reference implementation~~ — resolved in Phase 7: documented ranges only, no automated pass/fail gate, per the user's explicit choice after being shown what a hard gate would require (`picoface-phase7/design.md`, Decisions). `README.md`'s "What to expect" section publishes the current numbers (CNN held-out ~0.98–1.00, VAE held-out ~0.95–1.00, reconstruction ~0.8–1.0 for most classes, `classify_generated()` overall ~0.81 — the final Phase 6 + `capstone-linkage-sampling-fix` numbers, superseding the mid-Phase-6 figures this entry previously cited) and states plainly that nothing in the repo enforces them.
...
- Whether Phase 3's `build_autoencoder()` step is walked through in the generator arm's template notebook, or the notebook goes straight to `build_vae()` — **deferred, not resolved:** template notebooks themselves were downgraded to a post-MVP nice-to-have in Phase 7's scoping (`picoface-phase7/design.md`, Non-Goals) and not built, so this question stays open for whoever builds them. (`README.md`'s Quickstart, Phase 7's stand-in for notebook content, does note `build_autoencoder()` exists without walking through it, but that's not a decision about notebook content.)
...
- Whether `viz` gains helpers to show a `GeneratedImagesReport`'s images and an `activation_maximize()` image, or the capstone notebook plots them directly — **deferred, not resolved:** this is downstream of the capstone notebook itself, which Phase 7 did not build (see above). `viz` still only has `plot_training_history()`; no new helper was added.
- ~~Final distribution channel (PyPI vs. zip vs. Colab git-clone)~~ — resolved in Phase 7: PyPI is the chosen target/primary channel, per the user's stated preference (`picoface-phase7/design.md`, Decisions). The current git-clone-or-zip + `pip install -e .` path remains documented and accurate as what actually works today; `RELEASING.md` documents the not-yet-performed steps to actually publish.
- ~~**New from Phase 6:** `star`'s `classify_generated()` (cluster-sampled generation) stays weak even after loss-balance, `latent_dim`, and decoder-capacity tuning, though its reconstruction (decoding a real image's own latent) is now solid — the gap points at `star`'s latent cluster being more diffuse than other classes' (plausibly from its near-rotational symmetry), not at decoder capacity.~~ — resolved in Phase 7: measured the leading hypothesis (a rotation-driven ring-shaped latent cluster) directly and it didn't hold up — `star`'s cluster geometry looks like an ordinary blob, same as `circle`/`square`'s. The real cause was that the parametric class *mean* itself is a smeared multi-orientation composite that the decoder renders poorly, regardless of sampling spread or covariance shape (both tried and failed). Fix: `_class_latent_clusters`/`_sample_near_clusters` (`src/picoface/_internals/linkage_internals.py`) now resample a real encoded point per class plus small Gaussian jitter (`CLUSTER_SAMPLE_JITTER=0.1`) instead of drawing from a fitted Gaussian — every class improved, none regressed, and `star`/`smiley` closed most of their gap (`capstone-linkage-sampling-fix/design.md`).
