...
- [ ] 1.5 Update `dataset_forge/README.md` (shade and polarity description near line 129). Re-export the default config with seed 0, and run `python -m dataset_forge.contact_sheet` to look at it. Verify: validation passes; the contact sheet shows every class, including `negative_smiley`, as dark on light; the user reviews the contact sheet.
...
- [ ] 2.2 Run the smoke check for seeds 0, 1, 2 on the new default export, and for seed 0 on the old-polarity export (regenerate it from the pre-change commit or keep `default-2x-seed0`), with `--figures`. Record the results, per-class tables, and the edge-darkening comparison in this change's `diagnostics.md`. Verify: `diagnostics.md` holds both runs, and the flipped export's mean CNN and VAE held-out accuracy is within 0.01 of, or above, the old polarity's.
...
- [ ] 3.2 Create `_internals/picture_internals.py`:
  ...
  - picture → 2-D uint8 array with the strict grayscale check and its guidance error;
...
- [ ] 3.5 Implement `scale_down(image, size=28)` with `Image.Resampling.BOX` (design.md Decision 5). Verify:
  ...
  - the input kind is preserved and the input is unchanged.
...
- [ ] 4.1 Add a "Using picoface with mediaComp" section to `README.md`:
  - the draw → grayscale (the student's own loop) → `crop_and_center` → `scale_down` → `predict` walkthrough;
